...
- `ai-agent.techcloudup.com` 에서 누구나 바로 실행해볼 수 있는 라이브 데모 제공
...
| 도메인 | Cloudflare DNS → GCP 도메인 매핑 → Cloud Run | CNAME `ai-agent` → `ghs.googlehosted.com` |
| UI | Streamlit | `>=1.35.0` |
...
ai-agent/
├── app/
│   ├── main.py               # Streamlit 진입점 (st.navigation 멀티페이지)
│   ├── agent_client.py       # ADK Agent 호출 래퍼
│   └── pages/
│       ├── audit.py          # IAM Audit 채팅 UI
│       ├── about.py          # 소개 페이지
│       └── how_it_works.py   # 아키텍처 + ReAct 흐름 설명
├── agents/
...
- [x] 자연어 입력 → GCP IAM 실제 API 호출 → 리포트 생성 end-to-end 동작 확인
- [ ] `ai-agent.techcloudup.com` 에서 외부 접속 + 실행 가능 ← DNS 전파 대기 중
- [x] Agent의 Reasoning trace(생각 과정)가 UI에 실시간으로 표시됨
- [x] ADK multi-agent (Supervisor + Sub-agent) 구조 실제 동작
- [x] GitHub README에 Architecture diagram + 데모 스크린샷 포함
...
- [x] GitHub 레포 생성 (`ai-agent`) 및 초기 커밋
...
- [x] `app/agent_client.py` — Agent 호출 래퍼 작성
- [x] `app/main.py` — 기본 Chat 인터페이스 작성
- [x] 사이드바: GCP 프로젝트 ID 입력 필드 + 예시 프롬프트 버튼
- [x] 메인: Reasoning trace 실시간 표시 구현
- [x] 결과: Markdown 리포트 렌더링 확인
- [x] `streamlit run app/main.py` 로컬 동작 최종 확인

### Phase 6 — 컨테이너화 & 배포

- [x] `Dockerfile` 작성 및 `docker build` 로컬 테스트
- [x] `docker run` 으로 컨테이너 정상 동작 확인
- [x] `terraform/main.tf` 작성 (Cloud Run, IAM, Secret Manager)
- [x] `terraform init && terraform plan` 확인
- [x] `terraform apply` — Cloud Run 서비스 프로비저닝
- [x] Secret Manager에 환경변수 등록
- [x] Cloud Run 배포 후 URL 정상 접속 확인

### Phase 7 — CI/CD + 도메인

- [x] GitHub Actions `deploy.yml` 작성
- [x] Workload Identity Federation 설정 (SA key 없이 배포)
- [x] main 브랜치 push → 자동 배포 동작 확인
- [x] Google Search Console: `techcloudup.com` 도메인 소유권 인증 완료
- [x] GCP 도메인 매핑 생성: `ai-agent.techcloudup.com` → `ghs.googlehosted.com`
- [ ] Cloudflare DNS: CNAME `ai-agent` → `ghs.googlehosted.com` 으로 변경 (기존 Cloud Run URL에서)
- [ ] HTTPS 접속 및 외부 실행 최종 확인 (`curl -I https://ai-agent.techcloudup.com`)

### Phase 8 — 문서화

- [x] `README.md` 작성 (프로젝트 설명, Architecture diagram, 시작 가이드)
- [x] 데모 스크린샷 또는 GIF 캡처해서 README에 삽입
- [x] Streamlit 멀티페이지 추가: About + How it Works 페이지
- [ ] GitHub Topics 설정: `agentic-ai` `vertex-ai` `google-adk` `gemini` `gcp` `cloud-run` `python` `streamlit`
- [ ] 완성 기준 5개 항목 전체 체크 완료 확인
